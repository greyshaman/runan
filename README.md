...
<center><img src="https://github.com/greyshaman/runan/raw/refs/heads/dev/images/runan_3d_l.webp" width="50%" alt="Runen Logo"></center>

Цель этого проекта — создать модель, которая моделирует работу естественной нейронно-астроцитарной сети (NAN), подобной той, что функционирует в человеческом мозгу. Есть потребность понять, как работает логика в таких  NA сетях, и изучить различия между искусственными и естественными нейронными сетями в рамках модели. Модель может быть использованна для проверки математического подхода для сетей с нелинейным распространением сигнала и построения системы Искусственного Обощённого Интеллекта.
...
- [Описание](#описание)
  - [Модель нейрон-астроцитарной NA сети](#модель-нейрон-астроцитарной-na-сети)
    - [Нейрон](#нейрон)
    - [Астроцит](#астроцит)
    - [Трипартитный синапс](#трипартитный-синапс)
    - [Динамика синапса](#динамика-синапса)
      - [Роль астроцита в динамике синапса](#роль-астроцита-в-динамике-синапса)
    - [Кальциевые волны](#кальциевые-волны)
  - [Архитектура сети](#архитектура-сети)
- [Зависимости](#зависимости)
- [Как использовать](#как-использовать)
  - [Создание NA сети](#создание-NA-сети)
...
Моделирование нейрон-астроцитарных сетей на Rust.

Распределение и обработка сигналов в сети нейронов — увлекательный, но сложный процесс. Однако исследователи структуры головного мозга давно осознали ограниченность классических моделей, учитывающих только нейроны. Как оказалось, существенный вклад в распространение и модуляцию волн возбуждения (потенциалов действия) вносят глиальные клетки — астроциты.

Предлагаемая библиотека реализует модель, основанную на принципах биологического взаимодействия между нейронами и астроцитами, а также процессах, происходящих в трипартном синапсе. В модели участвуют два типа агентов:

* Нейроны — генерируют и передают потенциалы действия.

* Астроциты — модулируют синаптическую передачу, буферизуют ионы и нейромедиаторы, а также обмениваются кальциевыми сигналами.

Участники сети обмениваются сигналами разных типов (возбуждающими, тормозными, модуляторными). Сила возбуждения определяется амплитудой входного сигнала, поданного на сеть. Далее сигнал распространяется, обрабатывается и может:

* достичь одного или нескольких выходов сети,

* быть полностью подавлен (например, за счет тормозных интернейронов или астроцитарного захвата глутамата),

* либо циркулировать во внутренних рекуррентных контурах, формируя кратковременную память или ритмическую активность.

### Модель нейрон-астроцитарной NA сети

#### Нейрон

**Нейрон** является основным вычислительным элементом сети. Он состоит из:

* Дендритов с синапсами, принимающих входящие сигналы.

* Аксона, передающего обработанные сигналы другим участникам сети.

Дендриты действуют как входные порты, принимая сигналы от других нейронов. Эти сигналы накапливаются в теле нейрона (сумматоре), формируя мембранный потенциал. Аксон генерирует выходной сигнал, когда накопленный потенциал достигает порога возбуждения или когда завершается период интеграции (асинхронный сброс). Аксон может образовывать множество синаптических контактов с разными нейронами.

#### Астроцит

**Астроцит** — это глиальная клетка, которая:

* обеспечивает структурную поддержку нейронов,

* модулирует активность нейронных кластеров,

* взаимодействует с другими астроцитами через кальциевые волны,

* создает медленную обратную связь, реагируя на активность ассоциированных нейронов.

#### Трипартитный синапс

**Трипартитный синапс** в NAN представляет собой трехсторонний контакт между:

* пресинаптическим аксоном,

* постсинаптическим дендритом,

* отростком астроцита (филоподией).

Синапс характеризуется тремя параметрами:

* максимальная проводимость (потолок передачи),

* текущая проводимость (доступный ресурс),

* скорость восстановления проводимости (рекуперация).

#### Динамика синапса

**Динамика синапса:**

1. Входящий сигнал может пройти через синапс, но не более текущей проводимости (ограничение).

2. После прохождения сигнала текущая проводимость уменьшается на величину сигнала (истощение).

3. Со временем проводимость восстанавливается со скоростью регенерации до максимального значения.

**Дендрит** после синапса имеет **вес** — коэффициент, умножающий прошедший сигнал. Вес может быть положительным (возбуждение) или отрицательным (торможение).

**Нейрон** асинхронно обрабатывает сигналы от всех дендритов:

* Сигналы накапливаются в сумматоре.

* Сброс сумматора происходит при наступлении любого из событий:

  * Достижение порога возбуждения (генерация потенциала действия).

  * Истечение фиксированного интервала времени (базовый сброс).

* При сбросе нейрон:

  * Передает накопленное значение аксону (если оно положительно).

  * Записывает базовую активность (bias) в сумматор.

* Таким образом, нейрон может генерировать сигнал даже при нулевом входе.

**Аксон** передает положительный сигнал от сумматора во все подключенные к нему синапсы.

##### Роль астроцита в динамике синапса

Астроцит не участвует в передаче быстрых сигналов, но **модулирует скорость восстановления (регенерации) текущей проводимости** синапса. При высокой активности нейронов астроцит может:

* Увеличить скорость регенерации

* Уменьшить скорость регенерации

#### Кальциевые волны

Астроциты объединены в **синцитий** (через щелевые контакты). При активации нейронами астроцит генерирует волну повышенной концентрации Ca²⁺, которая распространяется к соседним астроцитам с затуханием. Волна изменяет модулирующее состояние группы астроцитов, создавая медленную пространственную обратную связь в сети.

#### Временная модель (план на будущее)

Планируется введение фазового параметра в сигналы для поддержки локальной синхронизации нейронных осцилляций, управляемой астроцитами.

### Архитектура сети

Сеть организована в слои нейронов, а также кластеры (синцитии) астроцитов. Сеть управляет созданием, конфигурацией и удалением слоев, а также подключением астроцитов к нейронам.

Входные порты соединяются с определенными синапсами нейронов. Выходные порты подключены к аксонам некоторых нейронов — с них можно считывать обработанные сигналы.

## Зависимости

Для реализации проекта применялись:

- Rust (начиная с 1.95.0)
- regex (1.11)
- tokio (1.41)
- tokio-stream (0.1)
- tokio-util (0.7)
